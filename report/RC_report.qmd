...
### Spatio-temporal model {#sec-model}
...
# consider for the fine-scale coral cover dynamics from monitoring data  

We addressed these criteria by modelling coral dynamics in space and time using a fixed-rank approach suitable for big data settings and non-Gaussian responses [@Zammit_Mangion_2021]. The model uses spatio-temporal random effects that are based on an automatic placement of spatio-temporal basis functions constructed via a tensor product of spatial and temporal basis functions (@fig-basis_function). These effects are spatially correlated to introduce a dependence (defined as spatial auto-correlation) between locations across three resolutions based on discrete global grids [@Sahr_2008]. Spatio-temporal random effects allow to interpolate values of coral cover at unobserved tier5 cells and thus increase the volume of information to interpret and scale-up predictions and uncertainty in an appropriate way. In this working example, the spatial component is composed of 120 functions. The temporal component uses 10 knots situated every 2.33 years years with a scaling factor of 2.1 (see [@Zammit_Mangion_2021] for additional information on the basis functions). 

```{r echo=FALSE}
#| label: fig-basis_function
#| fig-cap: Spatial (left) and temporal (right) basis functions used in the model.    
knitr::include_graphics(file.path(here::here(), "extra", "viz_basis_functions.png"), rel_path = FALSE)
```
...
Overall, the effect sizes of fixed effects are pretty similar between model formulation (@fig-fixed_eff). Lagged covariates contribute to reduce the uncertainty associated with the estimation of disturbance impacts with a substantive negative effects of cyclone exposure detected in the models that include them. The inclusion of random effects at the reef level decreases the range of predicted values of coral cover (@fig-range) and associated uncertainty (@fig-unc) at tier5 level. At tier4 level, lagged covariates reduce the magnitude of positive changes in coral cover between 2012-2018 and random effects reduce associated uncertainty (@fig-tier4). 
...
#| label: fig-tier4
#| fig-cap: Weigthed coral cover trajectory at tier4 level. The black line shows averaged area in coral cover and blue ribbons associated 95% credible intervals.  
...
::: {.callout collapse="true"}
### Basis function exploration analysis

The aim of this analysis is to explore the influence of the basis function formulation with a focus on the temporal dimension. To do this, we compare five model performance using different number of knots in the temporal basis function (@fig-all_basisfunction): 

* Model full: number and location of temporal knots automatically estimated from the FRK function and adopted by ReefCloud (@sec-model).
...
For each model, the spatial component is similar than model full (@fig-basis_function). At the tier5 level, the inclusion of additional knots tends to smooth estimated trajectory of coral cover and associated uncertainty (@fig-fit_basisfunction). Interestingly, this trend is not propagated at the tier4 level with a higher uncertainty estimated for the model full and lower for the model with one temporal knot and none (@fig-fit-tier4_basisfunction). Also, we estimate different effect size of disturbances across the models with a positive (but not significant) effect of cyclone exposure for the model full, five and three knots and negative (but not significant) otherwise (@fig-fixed-effect_basisfunction). Results show that Model full formulation produces the best model outputs based on the AIC values (@tbl-aic_basisfunction). 
   
```{r, include = TRUE, echo= FALSE}
#| label: fig-all_basisfunction
#| fig-cap: Locations of the knots for each temporal component. The x axis corresponds to the years.  
knitr::include_graphics(file.path(here::here(), "extra", "viz_basisfunction.png"), rel_path = FALSE)
```

```{r, include = TRUE, echo= FALSE}
#| label: fig-fit_basisfunction
#| fig-cap: Model fits for a unique tier5 with different temporal components. The black lines show averaged predictions at the tier5 level and blue ribbons associated 95% credible intervals. The grey lines show observed coral cover trajectories at the transect level. 
knitr::include_graphics(file.path(here::here(), "extra", "viz_pred_tier5_data_basisfunction.png"), rel_path = FALSE)
```

```{r, include = TRUE, echo= FALSE}
#| label: fig-fit-tier4_basisfunction
#| fig-cap: Weigthed coral cover trajectory at tier4 level. The black line shows averaged area in coral cover and blue ribbons associated 95% credible intervals.   
knitr::include_graphics(file.path(here::here(), "extra", "viz_pred_tier4_basisfunction.png"), rel_path = FALSE)
```

```{r, include = TRUE, echo= FALSE}
#| label: fig-fixed-effect_basisfunction
#| fig-cap: Estimated values of disturbance effects.   
knitr::include_graphics(file.path(here::here(), "extra", "fixed_effects_basisfunction.png"), rel_path = FALSE)
```

```{r, include = TRUE, echo= FALSE}
#| label: tbl-aic_basisfunction
#| tbl-cap: "AIC table."
#| tbl-colwidths: [60,40]
AIC_table <- read.csv("../extra/table_aic_basisfunction.csv")
colnames(AIC_table) <- c("Model name", "AIC")
kable(head(AIC_table))
```

```{r, include = TRUE, echo= FALSE}
#| label: tbl-time-basisfunction
#| tbl-cap: "Computing time (in min) for each test."
#| tbl-colwidths: [60,40]
time_table_leaveout <- read.csv("../extra/table_time_basisfunction.csv")
kable(head(time_table_leaveout))
```
:::
